...
- **Python virtual environment** (CRITICAL):
  ...
  - **Always** activate the venv first: `source .venv/bin/activate` or use the full path to the Python executable.
  - Correct command patterns:
    - `source .venv/bin/activate && python manage.py shell`
    - `.venv/bin/python manage.py shell` (direct path, no activation needed)
    - `.venv/bin/python -m pip install <package>`
  - **NEVER** run `python` or `python3` directly without activating the venv or using the full path — this will fail with "ModuleNotFoundError: No module named 'django'".
...
- **Examples (copyable patterns)**:
  ...
  - Django shell (direct path): `cd backend && ../.venv/bin/python manage.py shell`.
  - Install packages: `source .venv/bin/activate && pip install <package>` or `.venv/bin/pip install <package>`.
