...
if django_settings is not None:
    ...
    STORAGE_RESULTS_STREAM = getattr(
        django_settings,
        'DEVICEVAULT_STORAGE_RESULTS_STREAM',
        os.environ.get('DEVICEVAULT_STORAGE_RESULTS_STREAM', 'storage:results')
    )
else:
    ...
    STORAGE_RESULTS_STREAM = os.environ.get('DEVICEVAULT_STORAGE_RESULTS_STREAM', 'storage:results')
...
__all__ = ['app', 'BROKER', 'BACKEND', 'BROKER_API', 'REDIS_URL', 'RESULTS_STREAM', 'STORAGE_RESULTS_STREAM']
